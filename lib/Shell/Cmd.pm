...
sub _script {
   ...
   ######################################################################
   # First, we'll get a list of simple commands.  These are commands that
   # will show up in the simple script (i.e. the minimum necessary to do
   # the task without all of the fancy error handling and I/O redirection
   # on top of it.
   #
   # This generates a list of commands where each command is a hash
   # containing the command and a description of the command.
   ...
   ######################################################################
   # Now we'll turn these commands into a simple script.
   #
   # This is only needed if we will use a simple script (i.e. we're
   # being asked to produce a simple script, or on failure it shows
   # the simple script).  In other cases, this portion will be empty.
   ...
   ######################################################################
   # Now we'll generate the rest of a full script.
   #
   # This contains the simple commands fleshed out with all of the
   # other shell structure to build in all the desired functionality.
...
sub _simple_cmd {
   ...
   my $curr_flow = $self->_curr_flow();
   my $cmd_num   = $$self{'c'}{'c_num'};
   ...
   if (! ref($cmd)) {
      ...
      if      ($cmd =~ /^\s*(if)\s+.*?;\s*then\s*$/   ||
         ...
         if ($flow eq 'if') {
            ...
            if ($curr_flow ne 'if') {
               $$self{'err'} = "Broken flow: 'fi' found, but no 'if': $cmd_num";
               return ();
            }
...
sub _simple_script {
   ...
   my @script;
   return @script
     unless ($$self{'c'}{'g_fail'} eq 'display'  ||
             $$self{'c'}{'g_type'} eq 'simple');
   ...
   if ($$self{'c'}{'g_fail'} eq 'display') {
      push @script, $self->_script_segment
        (q[simple () {],
         q[   echo ""],
         q[   echo "#****************************************"],
   ...
   foreach my $tmp (@{ $$self{'c'}{'simple'} }) {
      ...
      if ($flow) {
         my ($cmd) = @cmd;
         if ($type eq 'open') {
            push @script,$self->_script_segment({ 'pref' => $pref },$cmd);
            $self->_ind_plus();
         } elsif ($type eq 'cont') {
            $self->_ind_minus();
            push @script,$self->_script_segment({ 'pref' => $pref },$cmd);
      ...
      push @script,$self->_script_segment({ 'pref' => $pref },"$cmd");
      foreach my $c (@cmd) {
         push @script,$self->_script_segment({ 'pref' => $pref },
                                             '# or',
                                             "# $c");
      }
...
#
# A simple script to print out a line and keep track of indentation.
# Only indentation relative to the current script segment is handled.
# If a line has greater/less indentation than the previous line, overall
# indentation increases/decreases by one level.
#
sub _script_segment {
   my($self,@lines) = @_;
   my @script;
   ...
   $lines[0]        =~ /^(\s*)/;
   my $curr_spc     = length($1);

   LINE:
   foreach my $line (@lines) {
      my %flags = ();
      if ($line =~ s,^<(.*?)>,,) {
         foreach my $cond (split(/,/,$1)) {
            if ($cond =~ /^(.*)=(.*)/) {
               $flags{$1} = $2;
            } else {
               $flags{$cond} = '';
      ...
      $line =~ s/^(\s*)//;
      my $line_spc  = length($1);
      if ($line_spc > $curr_spc) {
         $self->_ind_plus();
      } elsif ($line_spc < $curr_spc) {
         $self->_ind_minus();
      }
      $curr_spc = $line_spc;
      ...
      foreach my $key (keys %flags) {
         my $val = $flags{$key};
         if ($val) {
            next LINE  if (exists $$self{'c'}{$key} and
                           $$self{'c'}{$key} ne $val);
         } else {
            next LINE  if (exists $$self{'c'}{$key} and
                           $$self{'c'}{$key} eq '');
      ...
      if ($line) {
         ...
         push (@script,"${pref}${curr_ind}$line");
      } else {
         push (@script,'');
      }
   }
...
sub _script_init {
   ...
   my @script = $self->_script_segment('SC_FAILED=0;',
                                       'SC_EXIT=0;',
                                       'main () {');
   $self->_ind_plus();
   return @script;
...
sub _script_term {
   ...
   $self->_ind_minus();
   my @script = $self->_script_segment('}',
                                       '',
                                       'main;');
   ...
   if ($$self{'c'}{'g_fail'} eq 'display') {
      push (@script,
            $self->_script_segment('if [ $SC_FAILED -ne 0 ]; then',
                                   '   simple;',
                                   'fi'));
   }

   push @script, $self->_script_segment('exit $SC_EXIT;');
...
sub _flow {
   ...
   if ($$self{'c'}{'c_flow_type'} eq 'open') {
      push @script, $self->_script_segment("<g_type=run,g_echo=echo>" .
                                           "echo # $cmd",
                                           $cmd);
      $self->_ind_plus();
      ...
      $self->_ind_minus();
      push @script, $self->_script_segment($cmd,
                                           "<g_type=run,g_echo=echo>" .
                                           "echo # $cmd");
      $self->_ind_plus();
...
sub _cmd_init {
   ...
   if ($$self{'c'}{'c_retries'}) {
      my $retries = $$self{'c'}{'c_retries'};

      push @script,$self->_script_segment("SC_RETRIES=$retries;",
                                          'SC_TRY=0;',
...
sub _cmd_term {
   ...
   if ($$self{'c'}{'c_retries'}) {
      ...
      if ($$self{'c'}{'c_sleep'}) {
         my $sleep = $$self{'c'}{'c_sleep'};
         push @script,$self->_script_segment
           ('if [ $SC_TRY -lt $SC_RETRIES ]; then',
            "   sleep $sleep;",
            'fi');
      }

      $self->_ind_minus();
      push @script,$self->_script_segment('done');
   }
...
sub _alt_init {
      ...
      if ($$self{'c'}{'g_type'} eq 'script') {
         push @script,
           $self->_script_segment
             (qq(<g_out>echo "#SC CMD $cmd_num.$alt_num";),
              qq(<g_err>echo "#SC CMD $cmd_num.$alt_num" >\&2;));
         if ($$self{'c'}{'c_retries'}) {
            push @script,
              $self->_script_segment
                ('<g_out>echo "#SC TRY \$SC_TRY";',
                 '<g_err>echo "#SC TRY $SC_TRY" >\&2;');
         }
         ...
         if ($$self{'c'}{'c_retries'}) {
            push @script,
              $self->_script_segment
                ('#',
                 "# Command $cmd_num.$alt_num  [Retry: \$SC_TRY]",
                 '#');
         } else {
            push @script,
              $self->_script_segment
                ('#',
                 "# Command $cmd_num.$alt_num",
                 '#');
         }
      }
   }
   ...
   if ($$self{'c'}{'g_type'} eq 'run'  &&  $$self{'c'}{'g_echo'} eq 'echo') {
      push @script, $self->_script_segment(qq(echo "# $cmd";));
   }
...
sub _alt_term {
   ...
   if ($$self{'c'}{'g_type'} eq 'script') {
      push @script, $self->_script_segment('<g_out>echo "";',
                                           '<g_err>echo "" >&2;');
   }
...
sub _alt_cmd {
   ...
   $self->_ind_minus();
   push @script, $self->_script_segment('fi');
