...
Here, each $cmd is a string containing a command, or a listref where each
...
in the PER-COMMAND OPTIONS section below.
...
generates a script and runs it with the system() call.
...
If $obj is in 'script' mode, the output from the commands are kept for
...
where $out1 is a reference to the list of output on the 1st host, etc.
...
see the entry in GLOBAL OPTIONS below.
