...
Here, each C<$cmd> is a string containing a command, or a listref
where each element in the list is a command.
...
then handling can be much more complicated.  Refer to the
L</"FLOW COMMANDS"> section below.
...
the commands can be run, and these are described in the B<options>
method below.  The most important option is the B<mode> option which
sets the mode which determines the form of the script, and how it is
run.
...
If C<$obj> is in 'dry-run' mode, the method should be called as
either:
...
If C<$obj> is in 'script' mode, the output from the commands are kept
...
COMMAND_SUMMARY is a listref of the form:
   ...
   COMMAND_SUMMARY = [ CMD_NUM, STATUS,
                       ALT_SUMMARY_1,
                       ALT_SUMMARY_2, ... ]
...
STATUS is one of the strings:
   ...
   succ       STATUS of an empty string, or the string 'succ'.
              If the B<failure> option is set to 'continue', all
              commands which succeed are given the empty string until
              a command fails.  Commands run after that are given a
              STATUS of 'succ'.

              If B<failure> is set to any other option, only the empty
              string is ever assigned as STATUS for a successful
              command.

   retried  : If a command is retried, all attempts may fail (except
              the final one) and it is not counted as a script
              failure.  These failing commands are given this STATUS.
...
Each alternate will return a separate summary, also a listref, of the
form:
...
different way in order to deal with them properly.  This is because
the added functionality is done by wrapping each command with some
enclosing shell structure to add it.  It is very easy to wrap regular
shell commands in some enclosing structure.  For example, it is easy
to turn:
